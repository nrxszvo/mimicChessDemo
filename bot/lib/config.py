...
def insert_default_values(CONFIG: CONFIG_DICT_TYPE) -> None:
    ...
    set_config_default(
        CONFIG, key="pgn_file_grouping", default="game", force_empty_values=True
    )
    set_config_default(
        CONFIG, key="max_takebacks_accepted", default=0, force_empty_values=True
    )
    ...
    set_config_default(
        CONFIG, "challenge", key="allow_list", default=[], force_empty_values=True
    )
    set_config_default(
        CONFIG, "challenge", key="max_simultaneous_games_per_user", default=5
    )
    ...
    set_config_default(
        CONFIG, "matchmaking", key="overrides", default={}, force_empty_values=True
    )
...
def validate_config(CONFIG: CONFIG_DICT_TYPE) -> None:
    ...
    config_warn(
        ...
        "With challenge.concurrency set to 0, the bot won't accept or create "
        "any challenges.",
    )
    ...
    for setting in ["increment", "base", "days"]:
        ...
        config_warn(
            CONFIG["challenge"][f"min_{setting}"]
            <= CONFIG["challenge"][f"max_{setting}"],
    ...
    for db_name, valid_selections in selection_choices.items():
        ...
        db_section = (
            (CONFIG["engine"].get("online_moves") or {})
            if is_online
            else CONFIG["engine"]
        )
    ...
    explorer_config = (CONFIG["engine"].get("online_moves") or {}).get(
        "lichess_opening_explorer"
    )
    if explorer_config:
